import { Controller, Post, Body, Headers,Get, Param, Delete, UseGuards, Res,Req, HttpCode, HttpStatus } from '@nestjs/common';
...
export class AuthController {
  ...
  async requestPasswordReset(@Body() requestDto:requestDto): Promise<{ message: string }> {
    return this.authService.requestPasswordReset(requestDto);
  }

  @HttpCode(HttpStatus.OK)
  @Post('reset-password')
  async resetPassword(@Body() resetPasswordDto: ResetPasswordDto , @Headers('Authorization') token: string, ): Promise<{ message: string }> {
    const bearerToken = token.split(' ')[1];
